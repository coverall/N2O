...
class CC_Currency_Field extends CC_FloatNumber_Field
{
	...
	{
		return $this->currencySymbol . '<input type="text" id="' . $this->id . '"  size="' . $this->size. '" maxlength="' . $this->maxlength . '" name="' . $this->getRecordKey() . $this->name . '" value="' . htmlspecialchars($this->value) . '" class="' . $this->inputStyle . '"' . ($this->disabled ? ' disabled="true"' : '') . ($this->_onKeyup ? ' onKeyup="' . $this->_onKeyup . '"' : '' ) . '>';
	}
